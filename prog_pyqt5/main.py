...
def save_to_file():
    ...
    # start_date = QDate(2023, 7, 1)
    data_to_save = {'start': start_date, 'end': calc_date, 'desc': description}
    file1 = open('prog_pyqt5/config.txt', 'wb')
...
def read_from_file():
    ...
    try:
        file1 = open('prog_pyqt5/config.txt', 'rb')
...
form.label.setText("<html><head/><body><p align=\"center\">Трекер события от: \
                    %s</p></body></html>" % start_date.toString('dd-MM-yyyy'))
on_click_calendar()
